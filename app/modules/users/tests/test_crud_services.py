...
@pytest.fixture
def valid_user_dto():
    return UserSerializer(id=1, username="testuser", email="test@example.com", role="admin", password="hashed_password")
...
def test_create_user_with_existing_email(client, valid_user_dto, existing_user):
    duplicate_dto = UserSerializer(id=None, username="anotheruser", email=existing_user.email, role="admin", password="hashed_password")
...
def test_update_user(client, valid_user_dto):
    updated_dto = UserSerializer(id=valid_user_dto.id, username="updateduser", email="updated@example.com", role="admin", password="newhashedpassword")
...
def test_update_user_not_found(client):
    invalid_dto = UserSerializer(id=999, username="nonexistent", email="nonexistent@example.com", role="admin", password="password")
